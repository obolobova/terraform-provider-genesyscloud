...
// TestAccResourceOutboundCampaignPredictiveWithDiagnosticsSettings tests diagnostics_settings with predictive dialing mode
func TestAccResourceOutboundCampaignPredictiveWithDiagnosticsSettings(t *testing.T) {
	t.Parallel()
	var (
		resourceLabel            = "campaign_predictive_diag"
		name                     = "Test Predictive Campaign " + uuid.NewString()
		dialingMode              = "predictive"
		callerName               = "Test Name Predictive"
		callerAddress            = "+353371111112"
		contactListResourceLabel = "contact_list"
		queueResourceLabel       = "queue"
		locationResourceLabel    = "location"
		siteId                   = "site"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Create predictive campaign with diagnostics_settings block with explicit true
				Config: referencedResources +
					generateOutboundCampaign(
						resourceLabel,
						name,
...
// TestAccResourceOutboundCampaignPowerWithDiagnosticsSettingsOmitted tests default behavior when diagnostics_settings block is omitted
func TestAccResourceOutboundCampaignPowerWithDiagnosticsSettingsOmitted(t *testing.T) {
	t.Parallel()
	var (
		resourceLabel            = "campaign_power_no_diag"
		name                     = "Test Power Campaign No Diag " + uuid.NewString()
		dialingMode              = "power"
		callerName               = "Test Name Power"
		callerAddress            = "+353371111113"
		contactListResourceLabel = "contact_list"
		queueResourceLabel       = "queue"
		locationResourceLabel    = "location"
		siteId                   = "site"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Create power campaign without diagnostics_settings block - should use default (true)
				Config: referencedResources +
					generateOutboundCampaign(
						resourceLabel,
						name,
						...
						strconv.Quote("true"),
						generatePhoneColumnNoTypeBlock("Cell"),
						// No diagnostics_settings block - testing default behavior
					),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourcePath, "name", name),
					resource.TestCheckResourceAttr(resourcePath, "dialing_mode", dialingMode),
					// Default value should be true when block is omitted
					resource.TestCheckResourceAttr(resourcePath,
						"diagnostics_settings.0.report_low_max_calls_per_agent_alert", "true"),
				),
			},
